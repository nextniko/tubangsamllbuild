...
//
//
var _default = { data: function data() {return { status: { token: "", code: false, addstatus: true }, resdetail: { myinfo: {}, carList: {} } };}, onShow: function onShow() {this.status.token = uni.getStorageSync('token');this.api();}, methods: { api: function api() {var _this = this;uni.showLoading({ title: '加载中' });(0, _api.goUserDetail)({ token: this.status.token }).then(function (res) {uni.hideLoading();_this.status.code = res.code;if (res.code === "200") {_this.resdetail.myinfo = res.data;} else {// uni.navigateTo({
          // 	url:"/pages/login/index"
          // })
        }});(0, _api.carList)({ token: this.status.token }).then(function (res) {_this.status.code = res.code;if (res.code === "200") {_this.resdetail.carList = res.data;} else {// uni.navigateTo({
          // 	url:"/pages/login/index"
          // })
        }});}, gologin: function gologin() {uni.navigateTo({ url: "/pages/login/index" });}, //拨打电话
    tel: function tel() {uni.makePhoneCall({ phoneNumber: '400-806-1777' //仅为示例
      });}, gotoadd: function gotoadd() {if (this.status.addstatus) {this.status.addstatus = false;uni.navigateTo({
